...
const SubCategoryScreen = ({ match: { params } }) => {
  ...
  useEffect(() => {
    async function fetchData() {
      const response = await axios.get(
        `https://togedoorfirebase.herokuapp.com/subcategories/name/${subcategoryName}`
      );
  ...
  useEffect(() => {
    async function fetchData() {
      if (mySubCategory) {
        const response = await axios.get(
          `https://togedoorfirebase.herokuapp.com/services/getBySubCategory/${mySubCategory._id}`
        );
        ...
        <div>
          <Row>
            <Container style={{ maxWidth: "1375px" }}>
              <Breadcrumb>
                <Breadcrumb.Item active>{params.categoryName}</Breadcrumb.Item>
